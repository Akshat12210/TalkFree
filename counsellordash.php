...
//code to check all session variables present
...
while ($row = $result7->fetch_assoc()) {
        ...
        if (strtotime($current_datetime) > strtotime($send_date)) {
            ...
            if ($row['Completed'] != 1) {
                echo "<td><a href='completed.php?id=" . $row["ID"] . "'><button class='delete-acc bttn' type='button'>Completed</button></a> <a href='delete.php?id=" . $row["ID"] . "'><button class='delete' type='button'>Delete</button></a></td>
                </tr>";}
        }
    }
    ?>
