...
$sql9="SELECT DISTINCT Counsellor_Email FROM appointment where Counselle_Email='$email'";
$result10 = mysqli_query($conn, $sql9) or die("Your query is not right");
                        ...
                        <div class="form-popup" id="myForm">
                            <form action="action_page.php" class="form-container" method="post">
                                ...
                                <input class="date" type="date" id="date" name="date" min="<?php echo date("Y-m-d"); ?>"
                                    required />
                                <br />
                                <input class="time" type="time" id="time" name="time" step="1800" required />
                                <label class="check-head" for="">Choose the way you want to talk.</label>
                                <div class="talk-component">
                                    <input type="radio" checked="checked" name="remember" value="Video Call" /><span
                                        class="text-lg">Video call</span>
                                ...
                                <div class="select-user">
                                    <label class="User-select" for="User">Counsellor :</label>
                                    <select name="category" id="select-user">
                                        <option value="Select">-Select-</option>
                                        <option value="random">Random</option>
                                        <?php 
    while ($row10 = $result10->fetch_assoc()){
        $email_counsellor=$row10['Counsellor_Email'];
        $sql_q = "SELECT * FROM users WHERE Email='$email_counsellor'";
        $resultq = mysqli_query($conn, $sql_q) or die("Your query is not right");
        $q = mysqli_fetch_assoc($resultq);
        echo "<option value='$email_counsellor'>".$q["Name"]."</option>";
...
?>
                                ...
                                <input class="desc-text" type="text" id="desc" name="desc" required />
